...
def cd(to):
	...
	if to == '-':
		return cd(Previous)
	if not hasattr(to,'cd'): to = makepath(to)
	previous = os.getcwd()
	if to.isdir():
		os.chdir(to)
	elif to.isfile():
...
class Path(path):
	...
	def _get_ext(self):
		f, ext = os.path.splitext(_base(self))
		return ext
	ext = property (_get_ext,None,None )


	# pass on some methods to os and os.path
	basename = os.path.basename
	...
	# Pass on some methods to path, as we are using containment
	def open(self,mode='r'):
		return path(self).open(mode)
	def dirs(self,pattern=None):
		return [Path(p) for p in path(self).dirs(pattern)]
	def walkdirs(self,pattern=None):
		return [Path(d) for d in path(self).walkdirs(pattern)]
	def files(self,pattern=None):
		return [Path(f) for f in path(self).files(pattern)]
	def walkfiles(self,pattern=None):
		return [Path(f) for f in path(self).walkfiles(pattern)]
	def walk(self,pattern=None):
		return [Path(d) for d in path(self).walk(pattern)]
	def splitall(self):
		return [Path(p) for p in path(self).splitall()]
	def joinpath(self,*args):
		return Path(path(self).joinpath(*args))
	def readlink(self):
		return Path(path(self).readlink())
	...
	def filenames(self):
		return [ f.namebase for f in self.files() ]

	def let_file_exist(self):
		self.parent.makeDirExist()

	def makeDirExist(self,force=False):
		if self.isdir():
			return False
	...
	def tryRemove(self):
		if self.isfile():
			...
			sh.hide('rm -rf %s' % self)
		elif self.islink():
			sh.hide('unlink %s' % self)
		else:
			return False
		return sh.passed()
...
def makepath (short=None):
	'''make a path object from the expanded path string'''
	if isinstance(short,Path):
		return short
	if short is None:
		short = '.'
	short = str(short)
	expanded = os.path.normpath(os.path.expandvars(os.path.expanduser(short)))
	longPath = os.path.abspath(os.path.realpath(expanded))
	return Path(longPath)

def ancestor(path,sought):
	'''Get a path which is an ancestor of the given path, whose name is sought.

	Look at each individual directory name in the path, in reverse order.
	If any such name equals sought, then the path to there is returned.
	If no such name matches, then a PathError is raised.

	Examples:
		p = path('/mnt/source/builds/4.0/altoBSC/source/fred.c')
		ancestor(p,'builds') -> path('/mnt/source/builds/')
		ancestor(p,'source') -> path('/mnt/source/builds/4.0/altoBSC/source/')
		ancestor(p,'build') -> PathError
	Note:
		ancestor(ancestor(p,'source'),'source') -> path('/mnt/source/builds/4.0/altoBSC/source/')
		ancestor(ancestor(p,'source').parent,'source') -> path('/mnt/source/')
	'''
	parts = path.splitall()
	parts.reverse()
	for i, part in enumerate(parts[1:]):
		if re.match(sought,part):
			break
	else:
		return None
	i = len(parts) - (i + 1)
	parts.reverse()
	ancestorParts = parts[:i]
	return path.joinpath(*ancestorParts)
