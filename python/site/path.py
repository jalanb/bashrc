...
class path(_base):
	...
	def chmod(self, mode):
	    log('chmod %s %s' % (mode,self))
	    os.chmod(self, mode)

	if hasattr(os, 'chown'):
	    def chown(self, uid, gid):
	        log('chown %s:%s %s' % (uid,gid,self))
	        os.chown(self, uid, gid)

	def rename(self, new):
	    log('rename %s %s' % (self,new))
	    os.rename(self, new)
	...
	if hasattr(os, 'link'):
	    def link(self, newpath):
	        ...
	        log('ln %s %s' % (self,newpath))
	        os.link(self, newpath)

	if hasattr(os, 'symlink'):
	    def symlink(self, newlink):
	        """ Create a symbolic link at 'newlink', pointing here. """
	        log('ln -s %s %s' % (self,newlink))
...
class Path(path):
	...
	def assertExists(self):
	    if not self.exists(): raise PathError('%s does not exist' % self)
	    return self
...
def makepath (p):
	...
	try:
		return Path(p.expand().realpath().abspath())
	except AttributeError:
		p = path(p)
		return Path(p.expand().realpath().abspath())
...
def ancestor(path,sought):
	...
	parts.reverse()
	try:
	    i = parts.index(sought)
	except ValueError:
	    raise PathError('"%s" is not an ancestor of path("%s")' % (sought,path))
	i = len(parts) - i
	parts.reverse()
