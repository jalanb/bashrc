...
class ParseError(Exception):
    ...
    def formatReason(self):
        if not self.error: return 'looks like Left Recursion'
        if len(self.error) == 1:
            ...
            for s in self.error:
                if s[2] is None:
                    ...
                    if s[1] is not None:
                        desc = "%s %s" % (s[1], desc)
                bits.append(desc)
            return "expected one of (%r, %r)" % (', '.join(bits[:-1]), bits[-1])
    ...
    def formatError(self, input):
        ...
        prev = ''
        pprev = ''
        try: position = int(self.position)
        except: position = 0
        for line in lines:
...
def eof():
    ...
    """
    return [("message", "end of input", None)]
