...
case "$extension" in
	...
	py|sh|c|cpp|js|java|sh|html)
		...
		then
			...
			then
				pygmentize "$path" | head -n $maxln | python $(dirname $BASH_SOURCE)/chop.py -w $width && exit 4
			else
				pygmentize "$path" | head -n $maxln | fmt -s -w $width && exit 4
			fi
		fi
esac
...
if [[ -z "$extension" ]]
then
	if head -n 1 "$path" | grep -q python
	then pygmentize -l python  | head -n $maxln | fmt -s -w $width && exit 4
	elif head -n 1 "$path" | grep -q sh\$ 
	then pygmentize -l sh  | head -n $maxln | fmt -s -w $width && exit 4
	fi
