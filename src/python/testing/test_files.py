...
def _get_path_to_test_directory(strings):
    ...
    for string in strings:
        path_to_string = sites.paths.path(string)
...
def _existing_test_files(path_to_stem):
    ...
    >>> _existing_test_files(sites.paths.path('/test_files.txt')) == []
    True

    But we should expect them for this script
    >>> __file__ in _existing_test_files(sites.paths.path(__file__))
    True
    """
...
def _get_path_stems(strings, recursive):
    ...
    paths = ['/' in string and sites.paths.path(string) or here / string for string in strings]
    if recursive:
