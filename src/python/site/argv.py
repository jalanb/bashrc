...
def _listize(method):
    ...
    def to_options(tuples):
        ...
        for tupel in tuples:
            ...
            options.append(method(long, help, default, short, action))
        return options
    return to_options
...
def _default_tuples():
    return [
        ('pdb', 'Use a python debugger', False, 'U'),
    ]


_added_tuples = _default_tuples()
...
def _make_parser(tuples=None):
    ...
    for tupl in tuples + _added_tuples:
        try:
            ...
            options.append(_make_argv_option(*five_tuple))
        except:
...
def _find_files_in_args(args):
    ...
    directories, files, _ = site.paths.split_directories_files(args)
    first_directory = directories and directories[0] or site.paths.path('.')
